...
// --- EXCEL DINÁMICO (ACTUAL O HISTÓRICO) ---
app.get('/api/ventas/excel/:id', verifyToken, async (req, res) => {
    try {
        ...
        // Definir si buscamos lo actual o un histórico
        if (cierreId === 'actual') {
        ...
        // Totales al final
        const sumaVentas = ordenes.reduce((acc, o) => acc + o.total, 0);
        datosVentas.push({ Fecha: '', Cliente: '--- TOTAL VENTAS ---', Total: sumaVentas });
        ...
        const sumaGastos = gastos.reduce((acc, g) => acc + g.monto, 0);
        datosGastos.push({ Fecha: '', Descripcion: '--- TOTAL GASTOS ---', Monto: sumaGastos });
